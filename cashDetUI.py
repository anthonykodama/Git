...
while(True):
    ...
    if len(live_results) > 0: #if a cash object is detected the following code will proceed
        ...
        if (conf_score) > 0.8: #Only provides text/speech output when confidence score is greater than 0.8
            cv2.putText(frame, f'Value: ${class_name} Accuracy: {round(conf_score * 100):02}%', (50, 50), cv2.FONT_HERSHEY_SIMPLEX, 1, (0,0,0), 4) #Bill label output, black Outline
            cv2.putText(frame, f'Value: ${class_name} Accuracy: {round(conf_score * 100):02}%', (50, 50), cv2.FONT_HERSHEY_SIMPLEX, 1, (255,255,255), 1) #Bill label output, white text
